...
const GITHUB_URL_RE = /https?:\/\/github\.com\/(?<owner>[A-Za-z0-9_.-]+)\/(?<repo>[A-Za-z0-9_.-]+)(?:\/(?<section>issues|pull|pulls|commit|releases\/tag)\/(?<item>[^\s<>'"\])}，。；、]+))?/gi;
...
function githubArtifactFromMatch(match, url = "") {
...
export function extractArtifacts(text) {
    ...
    for (let match = URL_RE.exec(source); match; match = URL_RE.exec(source)) {
        if (githubUrlSpans.some(([start, end]) => start <= match.index && match.index < end))
            continue;
...
function artifactLabel(artifact) {
    ...
    if (artifact.kind === "github_issue")
        label = `GitHub issue ${repo}#${artifact.number}`;
    else if (artifact.kind === "github_pull")
        label = `GitHub PR ${repo}#${artifact.number}`;
    else if (artifact.kind === "github_issue_or_pull")
        label = `GitHub issue/PR ${repo}#${artifact.number}`;
    else if (artifact.kind === "github_commit")
        label = `GitHub commit ${repo}@${String(artifact.commit || "").slice(0, 12)}`;
    else if (artifact.kind === "github_release")
        label = `GitHub release ${repo} ${artifact.tag}`;
    else if (artifact.kind === "github_repo")
        label = `GitHub repo ${repo}`;
    else
        label = `URL ${url}`;
    if (url && !label.includes(url))
        label = `${label} (${url})`;
    return label.length > 240 ? `${label.slice(0, 239).trimEnd()}…` : label;
}
export function artifactAnchorBlock(artifacts) {
    if (!artifacts.length)
        return "";
    return `${ANCHOR_MARKER} ${artifacts.slice(0, 10).map(artifactLabel).join("; ")}`;
}
export function enrichContentWithArtifactAnchors(content) {
    const cleaned = String(content || "").trim();
    if (!cleaned || cleaned.includes(ANCHOR_MARKER))
        return cleaned;
    const block = artifactAnchorBlock(extractArtifacts(cleaned));
    return block ? `${cleaned}\n\n${block}` : cleaned;
}
export function mergeArtifactMetadata(metadata, content) {
    ...
    const artifactEntities = [];
    for (const artifact of artifacts) {
        if (artifact.repo)
            artifactEntities.push(artifact.repo);
        if (artifact.repo && artifact.number)
            artifactEntities.push(`${artifact.repo}#${artifact.number}`);
        if (artifact.repo && artifact.tag)
            artifactEntities.push(`${artifact.repo}@${artifact.tag}`);
        if (artifact.repo && artifact.commit)
            artifactEntities.push(`${artifact.repo}@${artifact.commit.slice(0, 12)}`);
    }
    ...
    const tags = ["artifact"];
    if (artifacts.some((artifact) => artifact.kind.startsWith("github_")))
        tags.push("github");
