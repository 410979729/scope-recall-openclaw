...
const CLAIM_RE =
  /^\s*(?<subject>.+?)\s+(?<predicate>is\s+not|are\s+not|was\s+not|were\s+not|does\s+not\s+use|do\s+not\s+use|doesn't\s+use|don't\s+use|no\s+longer\s+uses?|is|are|was|were|uses?|prefers?|likes?|wants?)\s+(?<value>.+?)\s*[.!?。！？]?\s*$/i;
const DEPLOY_RE =
  /\b(?<subject>.+?\bdeploy(?:ment)?(?:\s+command)?)\s+(?<predicate>is\s+not|does\s+not\s+use|do\s+not\s+use|doesn't\s+use|don't\s+use|no\s+longer\s+uses?|uses?|is|=)\s+(?<value>[^.!?。！？]+)\s*[.!?。！？]?/i;
...
export interface GovernanceReviewCandidate {
  id: string;
  category: MemoryEntry["category"];
  scope: string;
  text: string;
  reasons: string[];
  suggestedAction: "review" | "promote" | "archive" | "merge" | "delete";
  confidence: number;
  state?: string;
  layer?: string;
  source?: string;
...
export function isReviewConflict(existingText: string, candidateText: string): boolean {
  const existing = parseClaim(existingText);
  const candidate = parseClaim(candidateText);
  if (!existing || !candidate) return false;
  if (existing.negated === candidate.negated) return false;
  if (semanticSimilarity(existing.subject, candidate.subject) < 0.45) return false;
  if (
  ...
  if (semanticSimilarity(existing.value, candidate.value) < 0.75) return false;
  return true;
}
...
function withConflictReviewMetadata(entry: MemoryEntry, relatedId: string) {
...
export async function recordConflictReviewRelations(
  store: MemoryStore,
  entry: MemoryEntry,
  scopeFilter: string[],
  options: { limit?: number } = {},
): Promise<{ conflictCount: number; conflictIds: string[] }> {
  ...
  const conflicts = candidates.filter((candidate) =>
    candidate.id !== entry.id && isReviewConflict(candidate.text, entry.text)
  );
  if (conflicts.length === 0) return { conflictCount: 0, conflictIds: [] };

  for (const candidate of conflicts) {
    await store.patchMetadata(
      candidate.id,
      withConflictReviewMetadata(candidate, entry.id),
      scopeFilter,
    );
  }
  ...
  const meta = conflictIds.reduce(
    (acc, conflictId) => withConflictReviewMetadata({ ...current!, metadata: stringifySmartMetadata(acc) }, conflictId),
    parseSmartMetadata(current.metadata, current),
  );
...
export function buildGovernanceReviewCandidates(
  entries: MemoryEntry[],
  options: { limit?: number; includeText?: boolean } = {},
): GovernanceReviewCandidate[] {
  const candidates: GovernanceReviewCandidate[] = [];
  for (const entry of entries) {
    const meta = parseSmartMetadata(entry.metadata, entry);
    const lifecycle = String(meta.lifecycle ?? "").trim().toLowerCase();
    const reasons: string[] = [];
    if (meta.needs_conflict_review) reasons.push("conflict_review");
    if (meta.state === "rejected") reasons.push("inactive_lifecycle:rejected");
    if (["superseded", "obsolete", "rejected"].includes(lifecycle)) reasons.push(`inactive_lifecycle:${lifecycle}`);
    if (meta.state === "archived" || meta.memory_layer === "archive" || lifecycle === "archived") reasons.push("archive_review");
    if (meta.memory_layer === "working" || entry.category === "other") reasons.push("local_or_working_scratch");
    if (meta.source === "auto-capture" && meta.confidence < 0.72) reasons.push("raw_or_low_confidence_auto_capture");
    if (meta.source === "legacy") reasons.push("legacy_metadata_review");
    if (meta.confidence < 0.55) reasons.push("low_confidence");
    if (!reasons.length) continue;

    const suggestedAction: GovernanceReviewCandidate["suggestedAction"] =
      reasons.includes("conflict_review")
        ? "review"
